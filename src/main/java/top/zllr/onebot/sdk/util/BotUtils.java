package top.zllr.onebot.sdk.util;

import top.zllr.onebot.sdk.action.BaseBot;
import top.zllr.onebot.sdk.entity.MsgSegment;
import top.zllr.onebot.sdk.enums.MsgType;
import top.zllr.onebot.sdk.event.message.MessageEvent;
...
public class BotUtils {
     ...
     * 判断是否为全体at
     *
     * @param msgSegment 消息链
     * @return 是否为全体at
     */
    public static boolean isAtAll(List<MsgSegment> msgSegment) {
        return msgSegment.stream().anyMatch(it -> "all".equals(it.getData().get("qq")));
    }
     ...
     * @return at对象列表
     */
    public static List<Long> getAtList(List<MsgSegment> msgSegment) {
        return msgSegment
                .stream()
     ...
     * @return 图片链接列表
     */
    public static List<String> getMsgImgUrlList(List<MsgSegment> msgSegment) {
        return msgSegment
                .stream()
     ...
     * @return 视频链接列表
     */
    public static List<String> getMsgVideoUrlList(List<MsgSegment> msgSegment) {
        return msgSegment
                .stream()
     ...
     * @return 消息链
     */
    public static List<MsgSegment> rawToArrayMsg(@NonNull String msg) {
        List<MsgSegment> chain = new ArrayList<>();
        try {
            Arrays.stream(msg.split(CQ_CODE_SPLIT)).filter(s -> !s.isEmpty()).forEach(s -> {
                ...
                MsgSegment item = new MsgSegment();
    ...
    /**
     * 从 MsgSegment 生成 CQ Code
     *
     * @param msgSegment {@link MsgSegment}
     * @return CQ Code
     */
    public static String arrayMsgToCode(MsgSegment msgSegment) {
        StringBuilder builder = new StringBuilder();
        if (Objects.isNull(msgSegment.getType()) || MsgType.unknown.equals(msgSegment.getType())) {
            builder.append("[CQ:").append(MsgType.unknown);
        } else {
            builder.append("[CQ:").append(msgSegment.getType());
        }
        msgSegment.getData().forEach((k, v) -> builder.append(",").append(k).append("=").append(escape(v)));
        builder.append("]");
    ...
    /**
     * 从 List<MsgSegment> 生成 CQ Code
     *
     * @param msgSegments {@link MsgSegment}
     * @return CQ Code
     */
    @SuppressWarnings("SpellCheckingInspection")
    public static String arrayMsgToCode(List<MsgSegment> msgSegments) {
        StringBuilder builder = new StringBuilder();
        for (MsgSegment item : msgSegments) {
    ...
    public static void rawConvert(JsonObject rawJson, MessageEvent event) {
        ...
        if (rawJson.has("message")) {
            ...
            if (GsonUtils.isArrayNode(rawJson, "message")) {
                List<MsgSegment> msgSegment = GsonUtils.convertToList(message, MsgSegment.class);
                // 存入 array message
                event.setMsgSegment(msgSegment);
                ...
                if (rawJson.has("raw_message") && (event.getRawMessage() == null || event.getRawMessage().isEmpty())){
                    event.setRawMessage(arrayMsgToCode(msgSegment));
                }
            } else {
                event.setMsgSegment(rawToArrayMsg(message));
                if (rawJson.has("raw_message") && (event.getRawMessage() == null || event.getRawMessage().isEmpty())){
     ...
     * 生成自定义合并转发消息
     *
     * @param contents 消息列表，每个元素视为一个消息节点 Object 可为 List<MsgSegment> 或 CQCode
     * @return 消息结构
     */
    @SuppressWarnings("Duplicates")
